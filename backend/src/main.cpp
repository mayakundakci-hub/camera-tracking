...
int main(int argc, char** argv)
{
    ...
    placement::LatchPolicy latchPolicy;
    {
        ...
        latchPolicy.max_mean_error_m =
            frames::convert::mmToM(l.value("max_mean_error_mm", 2.0));
    ...
    comparison::Comparator comparator(sceneCfg, registry,
                                       cfg["comparison"]["max_match_gap_sec"],
    ...
    if (optitrackRequired)
    {
        ...
        const std::string localAddr  = cfg["optitrack"].value("local_address", std::string{"127.0.0.1"});
        if (!optitrack::start(
                motiveIp, localAddr, natnetClient,
                [&placer](const std::vector<placement::BodyObservation>& bodies, double stamp) {
                    placer.onMocapFrame(bodies, stamp);
                }))
        {
    ...
    if (fanucStubEnabled)
    {
        ...
        fanucPub = std::make_unique<middleware::EcalProtoPublisher<PosePacket>>("backend", topicFanuc);
        fanucThread = std::make_unique<std::thread>(fanuc::runStubLoop, fanucRateHz,
                                                     std::ref(*fanucPub));
    ...
    const std::string topicJointState = cfg["ecal"].value("topic_joint_state",
                                                           std::string{"robot/joint_state"});
    ...
    for (const auto& c : placer.scene().comparisons())
        std::printf("[backend/comparison] %s: %s vs %s\n",
                    c.object_id.c_str(), c.a.c_str(), c.b.c_str());
    ...
    for (const auto* p : placer.scene().placements())
    {
        ...
        std::printf("[backend/construct] %s: %s built from the chord %s -> %s\n",
                    p->object_id.c_str(), p->id.c_str(),
                    p->inputs[0].c_str(), p->inputs[1].c_str());
    }
    ...
    struct YawAccum { double sin{0.0}; double cos{0.0}; int n{0}; };
    ...
    while (eCAL::Ok())
    {
        ...
        // ---- the same angles, in the shape a URDF poser accepts ----
        {
            ...
            for (int i = 0; i <= highest; ++i) measured.add_hand_joints(0.0);
        ...
        for (const auto& [id, e] : placer.jointEstimates())
        {
            ...
            std::printf("[joint] %s: theta %.3f deg, radial %+.3f mm, axial %+.3f mm, "
                        ...
                        e.radial_error_m * 1000.0, e.axial_error_m * 1000.0,
                        e.residual_m * 1000.0, frames::convert::radToDeg(e.residual_rad),
                        e.confidence, (radialBad || axialBad) ? "   <-- CHECK GEOMETRY" : "");
        ...
        for (const auto& [id, e] : placer.jointEstimates())
        {
            ...
            std::printf("[joint] %s: replace the two position_mm arrays in 'joint' --\n"
                        ...
                        id.c_str(),
                        frames::convert::mToMm(zero.x()), frames::convert::mToMm(zero.y()),
                        frames::convert::mToMm(zero.z()),
            ...
            std::printf("[joint] %s: check the paste -- a turn about the axis preserves both the "
                        ...
                        "axis.\n",
                        id.c_str(),
                        frames::convert::mToMm(along(zero)), frames::convert::mToMm(across(zero)),
                        frames::convert::mToMm(along(axpt)), frames::convert::mToMm(across(axpt)));
        ...
        if (captureZero)
        {
            for (const auto& [id, e] : placer.jointEstimates())
            {
                ...
                std::printf("[joint] %s: if THIS pose is the joint's zero, replace BOTH halves --\n"
                            ...
                            "           }\n",
                            id.c_str(),
                            frames::convert::mToMm(pos.x()), frames::convert::mToMm(pos.y()),
                            frames::convert::mToMm(pos.z()),
        ...
        for (const auto* p : placer.scene().placements())
        {
            ...
            std::printf("\n[mounts] %s: '%s' -> '%s' is %.3f deg about [%.4f, %.4f, %.4f]\n",
                        ...
                        frames::convert::radToDeg(rel.angle()),
                        rel.axis().x(), rel.axis().y(), rel.axis().z());
        ...
        for (const auto& [id, r] : placer.constructionReports())
        {
            ...
            std::printf("[construct] %s: chord %.2f mm measured vs %.2f mm from CAD (%+.2f mm), "
                        "normals disagree %.4f deg, chord off plane %+.2f mm%s\n",
                        id.c_str(), r.chord_measured_m * 1000.0, r.chord_expected_m * 1000.0,
                        r.chord_error_m * 1000.0,
                        frames::convert::radToDeg(r.normal_disagreement_rad),
        ...
        for (const auto& [id, r] : placer.latchReports())
        {
            ...
            std::printf("[latch] %s: %zu admitted of %zu seen, %zu rejected (%.1f%%); "
                        "spread %.3f mm / %.4f deg, std_err %.4f mm / %.5f deg, "
                        "marker error %.3f mm -- latched\n",
                        id.c_str(), r.admitted, r.seen, r.rejected,
                        r.window ? 100.0 * static_cast<double>(r.rejected) /
                                       static_cast<double>(r.window)
                                 : 0.0,
                        r.spread_m * 1000.0, frames::convert::radToDeg(r.spread_rad),
                        r.std_err_m * 1000.0, frames::convert::radToDeg(r.std_err_rad),
                        r.mean_error_m * 1000.0);
        }
