...
class Comparator {
public:
    Comparator(scene::Scene scene, const frames::Registry& registry,
               double maxMatchGapSec, std::size_t bufferLen)
        : scene_(std::move(scene)), registry_(registry),
          maxMatchGapSec_(maxMatchGapSec), bufferLen_(bufferLen)
    {
        for (const auto* p : scene_.placements())
            if (isTimeVarying(*p))
                buffers_[p->id];   // create the (empty) deque
    }
...
private:
    [[nodiscard]] Delta evaluate(const scene::Comparison& c) const
    {
        ...
        const bool bothLatched = pa->capture == scene::Capture::Latched &&
                                 pb->capture == scene::Capture::Latched;
        ...
        if (bothContinuous)
        {
            if (!matchInTime(c.a, c.b, Ta, Tb, d))
                return d;   // matchInTime filled in the reason
        }
        else
        {
            Ta = registry_.lookup(scene_.anchor_frame, c.a);
            Tb = registry_.lookup(scene_.anchor_frame, c.b);
            if (!Ta) { d.invalid_reason = "'" + c.a + "' has not been placed yet"; return d; }
            if (!Tb) { d.invalid_reason = "'" + c.b + "' has not been placed yet"; return d; }
        }
    ...
    bool matchInTime(const std::string& a, const std::string& b,
                     std::optional<frames::Transform>& Ta,
                     std::optional<frames::Transform>& Tb, Delta& d) const
    {
        std::lock_guard<std::mutex> lock(mutex_);
        ...
        if (ia->second.empty()) { d.invalid_reason = "'" + a + "' has no samples yet"; return false; }
        if (ib->second.empty()) { d.invalid_reason = "'" + b + "' has no samples yet"; return false; }
        ...
        for (const auto& cand : ib->second)
        {
            ...
            if (!best || gap < bestGap) { best = &cand; bestGap = gap; }
        }
        ...
        if (bestGap > maxMatchGapSec_)
        {
            ...
            d.invalid_reason = "no time match: nearest sample is " +
                               std::to_string(bestGap) + "s away, limit " +
                               std::to_string(maxMatchGapSec_) + "s";
            return false;
        }
