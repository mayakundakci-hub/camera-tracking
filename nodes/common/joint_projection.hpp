...
struct RevoluteJoint {
    ...
    [[nodiscard]] frames::Transform at(const frames::FrameId& parent,
                                          ...
                                          bool& clamped)
{
    ...
    if (wrapped < lower) { clamped = true; return lower; }
    if (wrapped > upper) { clamped = true; return upper; }
    return wrapped;
}
...
inline frames::Transform RevoluteJoint::at(const frames::FrameId& parent,
                                           const frames::FrameId& child,
                                           double theta_rad, double stamp) const
{
                                        ...
                                        const frames::FrameId& correctedChildFrame)
{
...

    {
        ...
        out.radial_error_m = (dMeas - axialMeas * axis).norm() -
                             (dZero - axialZero * axis).norm();
    }
    ...
    out.theta_rad =
        clampToLimits(out.theta_raw_rad, joint.lower_rad, joint.upper_rad, out.clamped);

    out.corrected = joint.at(T_parent_measured.to, correctedChildFrame, out.theta_rad,
                             T_parent_measured.stamp);
...

    {
        ...
        {
            out.yaw_to_align_rad = std::atan2(axis.dot(predicted.cross(measured)),
                                              predicted.dot(measured));
