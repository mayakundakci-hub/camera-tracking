...
mu::robot::RobotVisual toRobotVisual(const ludus::RobotSceneVisual& v)
{
    ...
    {
        rv.has_color = true;
        rv.color     = {v.material->color.r, v.material->color.g, v.material->color.b,
                        v.material->color.a};
    }
    return rv;
}
                                   ...
                                   std::vector<mu::robot::RobotVisual>& rows)
{
    ...
    for (const ludus::RobotSceneVisual& v : built.visuals)
        rows.push_back(toRobotVisual(v));
                            ...
                            std::vector<mu::robot::RobotVisual>& rows)
{
    ...
    VisualOffset vo;
    vo.positionScene =
        QVector3D(static_cast<float>(t.x()), static_cast<float>(t.y()), static_cast<float>(t.z())) *
        kMetresToScene;
...
void RobotScene::poseArticulated()
{
    ...
    if (logging)
    {
        ...
        std::printf("[render] '%s': %zu visuals, root at [%.1f %.1f %.1f] mm\n",
                    articulatedId_.c_str(), built.visuals.size(), articulatedRootMm_.x(),
                    articulatedRootMm_.y(), articulatedRootMm_.z());
        std::printf("[render]   joints fed to FK:");
        for (double v : lastJoints_)
            std::printf(" %.3f", qRadiansToDegrees(v));
        std::printf(" deg\n");
    }
    ...
    for (const ludus::RobotSceneVisual& v : built.visuals)
    {
        ...
        if (logging)
            std::printf("[render]   %-24s FK [%8.1f %8.1f %8.1f] -> drawn [%8.1f %8.1f %8.1f] mm"
                        "  scale [%.3f %.3f %.3f]  %s\n",
                        v.name.c_str(), p[0], p[1], p[2], position.x(), position.y(), position.z(),
                        s[0], s[1], s[2], v.mesh_path.c_str());
                                  ...
                                  const QQuaternion& rotation, bool valid)
{
    ...
    if (!articulatedId_.empty() && placementId.toStdString() == articulatedId_)
    {
        if (valid)
        {
            ...
            if (haveSmoothedRoot_ && renderSmoothing_ < 1.0)
            {
                ...
                articulatedRootRot_ = QQuaternion::slerp(articulatedRootRot_, rot, a).normalized();
            }
            else
            {
    ...
    if (it == meshScale_.end()) return;   // no geometry for this placement (frame-only)
