...
public:
    explicit EcalLayer(AppState& state)
        ...
        , measuredJoints_("frontend", topic("topic_hand_joint_state_measured"),
                          [&state](const JointStatePacket& msg) {
                              state.updateMeasuredJoints(msg);
                          })
        , jointEstimates_("frontend", topic("topic_scene_joint_estimates"),
                          [&state](const JointEstimatePacket& msg) {
                              state.updateJointEstimates(msg);
                          })
