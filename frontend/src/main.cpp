...
int main(int argc, char** argv)
{
    ...
    int rc = 0;
    {
        ...
        robotScene.setRenderSmoothing(
            Config::load()["frontend"].value("render_smoothing", 0.25));
        ...
        QObject::connect(&backend, &Backend::viewportChanged, &robotScene, [&] {
            ...
            for (const auto& p : backend.placements().placements()) {
                ...
                robotScene.setPlacementPose(
                    ...
                    q.lengthSquared() > 1.0e-12f ? q.normalized() : QQuaternion(),
                    p.valid());
            }
        });
